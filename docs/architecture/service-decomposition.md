...
| 7 | Triage & Workflow | `triage` | Lifecycle transitions, FP workflow, risk acceptance, exceptions with expiry, ownership | `triage.finding.assigned` and `triage.finding.status_changed` (implemented), `triage.exception.created`, `triage.exception.expired` | `risk.finding.scored`, `normalization.finding.resolved` |
