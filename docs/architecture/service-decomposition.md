...
| 7 | Triage & Workflow | `triage` | Lifecycle transitions, FP workflow, risk acceptance, exceptions with expiry, ownership | `triage.finding.assigned` (implemented), `triage.finding.status_changed`, `triage.exception.created`, `triage.exception.expired` | `risk.finding.scored`, `normalization.finding.resolved` |
